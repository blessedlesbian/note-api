...
import { saveTags, getTagsDB, getNotesDB } from '../services/db.service.js';
...
export async function createTag(req, res) {
  ...
  if (db.find(t => t.userId === userId && t.name === name)) {
...
export async function listTags(req, res) {
  const userId = req.user.id;
  const db = await getTagsDB();
  const tags = db.filter(t => t.userId === userId);
  res.json(tags);
}
...
export async function updateTag(req, res) {
  ...
  if (!name) return res.status(400).json({ message: 'name required' });
  const db = await getTagsDB();
  const tag = db.find(t => t.id === id && t.userId === userId);
...
export async function deleteTag(req, res) {
  ...
  const userId = req.user.id;
  const db = await getTagsDB();
  const dbNote = await getNotesDB();
  const idx = db.findIndex(t => t.id === id && t.userId === userId);
  ...
  const tagId = id;
  const used = dbNote.find(n => n.userId === userId && n.tagIds.includes(tagId));
